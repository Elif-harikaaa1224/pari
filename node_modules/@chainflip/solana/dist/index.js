...
var tryDecodeCfParams = createVaultParamsDecoder(
  Bytes(32),
...
var bnSchema = z.any().refine((arg) => BN.isBN(arg)).transform((bn) => BigInt(bn.toString()));
var swapParams = z.object({
  amount: bnSchema,
  dst_chain: z.number(),
  dst_address: z.instanceof(Buffer),
  dst_token: z.number(),
  ccm_parameters: z.object({
...
var swapSchema = z.discriminatedUnion("name", [swapNativeSchema, swapTokenSchema]);
...
var getSolDiff = (tx, publicKey) => {
  ...
  assert(preBalance !== void 0, "failed to find preBalance");
  const postBalance = tx.meta.postBalances.at(accountIndex);
  assert(postBalance !== void 0, "failed to find postBalance");
...
export { ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, TransactionMatchingError, findTransactionSignatures, findVaultSwapData, findVaultSwapSignature, isValidSolanaAddress };
